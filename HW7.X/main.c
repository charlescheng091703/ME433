...
void blink();
void comp_filt(float* accel_n, float* accel_z, float* angvel_n, float* tilt, float dt, float a);
...
int main() {
    ...
    while (1) {
        ...
        for (i=0; i<NUM_DATA_PNTS; i++){
            ...
            while(_CP0_GET_COUNT()<120000){} // dt = 0.005 s
        }
        
        comp_filt(ax, az, gx, pitch, dt, a);
        comp_filt(ay, az, gy, roll, dt, a);
        
        // print data
        for (i=0; i<NUM_DATA_PNTS; i++){
            sprintf(m_out,"%d %f %f %f %f %f %f %f %f %f\r\n",NUM_DATA_PNTS-i,ax[i],ay[i],az[i],gx[i],gy[i],gz[i],temp[i],pitch[i],roll[i]);
            WriteUART1(m_out);
        }
...
void comp_filt(float* accel_n, float* accel_z, float* angvel_n, float* tilt, float dt, float a) {
    float tilt_accel = atan2f(*accel_n, *accel_z);
    float tilt_int = tilt_accel; 
    *tilt = tilt_accel;
    for (int i = 1; i < NUM_DATA_PNTS; i++) {
        tilt_accel = atan2f(*(accel_n + i), *(accel_z + i));
        tilt_int += *(angvel_n + i) * dt;
        *(tilt + i) = a*tilt_accel + (1 - a)*tilt_int;
    }
}
